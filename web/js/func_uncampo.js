...
function muestraDatos (Json) {
	...
	var Lacadena = '<select name="opcEdoCivil" id="edoCiv" onchange="grafica(\'OP1\')" class="form-control" required>';  
		Lacadena+= '<option selected="false" disable>Seleccione...</option>';
		llenaSelect (Lacadena, 'edoCiv',true);
	/*
...
function llenaSelect(cadena, xcampo, soloUn) {
	...
	//debugger;
	if (soloUn) {
			cadena+= '<option value="EdoCivil">Estado Civil</option>'
			cadena+= '<option value="RegimenC">Regimen Conyugal</option>'
			cadena+= '<option value="Ocupacio">Ocupación</option>'
			cadena+= '<option value="Escolari">Escolaridad</option>'
			cadena+= '<option value="NivelEco">Nivel Económico</option>'
		} else {
...
.attr("fill",colorRam)
.attr("font-family", "sans-serif")
.text(selId +"(" + selOpt + ") del " + anioS + " por " + radS);
*/ 
var idP = "L"+elId;
var leyenda = "<p id=" + idP + ">" +radS + "(" +anioS+")" + selOpt+"</p>";
...
function buscaNumMpio(strMpio) {
	...
	strMpio = temp;
	for (let x of lisMun){
		var a = strMpio.substring(0,7);
		var b = x.mun.substring(0,7);
		if (a == b) {
			regresa = x.N;
